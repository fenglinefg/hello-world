 ...
 return new Promise((resolve, reject) =>
   {
    const signurl = { 
     ...
     $.post(signurl, (error, response, data) =>{
      if(logs) $.log(`${CookieName}, data: ${data}`)
...
for (i=0;i<4;i++){
   if(100-rotaryres.data.remainTurn==rotaryres.data.chestOpen[`${rotarnum[i]}`].times){
   rotarbody = rotar+i+1
   const url = { 
