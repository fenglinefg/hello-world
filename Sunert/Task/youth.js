    ...
    return new Promise((resolve, reject) => {
        ...
        $.post(url, (error, response, data) => {
            ...
            if (punchcardstart.code == 1) {
                ...
                if(logs)$.log("每日报名打卡成功，报名时间:"+`${$.time('MM-dd HH:mm')}`)
                resolve();
            }
          else {
            //detail += `【打卡报名】${punchcardstart.msg}\n`
            if(logs)$.log(punchcardstart.msg)
            resolve()
          }
        })
    })
        ...
        setTimeout(() => {
            ...
            $.post(url, (error, response, data) => {
                ...
                if (punchcardend.code == 1) {
                  ...
                  if(logs) $.log("早起打卡成功，打卡时间:"+`${punchcardend.data.card_time}`)
                } else if (punchcardend.code == 0) {
                    // TODO .不在打卡时间范围内
                    //detail += `【早起打卡】${punchcardend.msg}\n`
                  if(logs) $.log("不在打卡时间范围内")
                }
                resolve()
            })
        },s)
            ...
            if (sharestart.code == 1) {
                setTimeout(() => {
                    ...
                    $.post(endurl, (error, response, data) => {
                        ...
                        if(logs) $.log(`【打卡分享】成功，获得`+shareres.data.score+"个青豆")
                        } else {
                            //detail += `【打卡分享】${shareres.msg}\n`
                         if(logs) $.log(`${shareres.msg}`)
                        }
                        resolve()
                    })
                  },s*2);
            ...
            $.post(url, (error, response, data) => {
                ...
                if (boxres.code == 1) {
                  ...
                  setdata(boxretime, 'opbox')
                    ...
                    if(logs) $.log(`开启时段宝箱成功，获得`+boxres.data.score+`个青豆，${boxres.data.time / 60}`+"后开启下一个宝箱")
                }else{
                    //detail += `【开启宝箱】${boxres.msg}\n`
                    if(logs) $.log(`${boxres.msg}`)
                }
                resolve()
            })
        },s)
...
function boxshare() {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            $.post(url, (error, response, data) => {
                if(logs) $.log(`开始分享宝箱任务`)
                shareres = JSON.parse(data)
                if (shareres.code == 1) {
                    detail += `【宝箱分享】+${shareres.data.score}青豆\n`
                  if(logs) $.log(`分享宝箱任务成功，获得${shareres.data.score}青豆`)
                }else{
    ...
    return new Promise((resolve, reject) => {
        ...
        $.post(url, (error, response, data) => {
            ...
            if (adVideores.status == 1) {
                ...
                if(logs) $.log("获得"+adVideores.score+"个青豆")
            }
            resolve()
        })
    })
    ...
    return new Promise((resolve, reject) => {
        ...
        $.post(url, (error, response, data) => {
            ...
            if (gameres.success == true) {
               ...
               if(logs) $.log("获得"+gameres.items.score)
            }else{
...
function readArticle() {
    return new Promise((resolve, reject) => {
        ...
        $.post(url, (error, response, data) => {
           ...
           if(logs) $.log(readres.items.max_notice)
            }else if (readres.items.read_score !== undefined) {
              detail += `【阅读奖励】+${readres.items.read_score}个青豆\n`;
              if(logs) $.log("本次阅读获得"+readres.items.read_score+"个青豆")
            }
            resolve()
        })
    })
    ...
    return new Promise((resolve, reject) => {
        ...
        $.post(url, (error, response, data) => {
            ...
            if (redres.success == true) {
              ...
              if(logs) $.log("惊喜红包获得"+redres.items.score+"个青豆")
            }else{
                if(redres.error_code == "100001"){
                    //detail += `【惊喜红包】${redres.message},疑似cookie没有\n`
                 if(logs) $.log("每日开启惊喜红包已超限")
                }
            }
            resolve()
        })
            ...
            $.post(url, (error, response, data) => {
               if(logs) $.log(`开始转盘任务`)
                ...
                if (rotaryres.status == 1) {
                    ...
                    if(logs) $.log("转盘抽奖获得"+rotaryres.data.score+"个青豆，转盘次数还有"+rotaryres.data.remainTurn+"次")
                }
                if (rotaryres.code == 10010) {
                    rotarynum = ` 转盘${rotaryres.msg}🎉`
                if(logs) $.log("转盘任务已全部完成")
                }else{
            ...
            $.get(url, (error, response, data) => {
             if(logs) $.log(`开始统计收益信息`)
                ...
                if (infores.status == 0) {
                 ...
                 if(logs) $.log('<今日合计>： ' + infores.history[0].score + " 青豆")
                }
                resolve()
            })
        },s)
