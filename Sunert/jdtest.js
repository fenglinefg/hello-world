...
!(async() => {
  ...
  // 下载最新代码
  await downFile();
  const content = await fs.readFileSync('./jingxi.js', 'utf8')
    ...
    if (cookie) {
      ...
      try {
        ...
        if ($.isNode()) {
          ...
          if (BarkContent) {
            await notify.BarkNotify(`账户${$.index} ${UserName}京喜签到`, `【签到时间】： ${beanSignTime}\n${BarkContent}`);
            BarkContent = BarkContent.replace(/[\n\r]/g, '\n\n');
            await notify.sendNotify(`账户${$.index} ${UserName}京喜签到`, `【签到时间】:  ${beanSignTime}\n\n${BarkContent}`);
          }
        }
        //运行完成后，删除下载的文件
        console.log('运行完成后，删除下载的文件\n')
        await deleteFile(path);
        await deleteFile(jingxiPath);
...
async function downFile () {
  ...
  const url = 'https://raw.githubusercontent.com/Sunert/Scripts/master/Task/jingxi.js';//pc端测试用
  await download(url, './')
}

async function changeFile (content) {
  const newContent = content.replace(/var Key = ''/, `var Key = '${cookie}'`);
  await fs.writeFileSync( './jingxi.js', newContent, 'utf8')
}
